...
Sur le site de Paris, une salle serveur va être créée et équipée 4 serveurs. Le cœur du réseau y sera installé. Les salariés du service R&D sont dans un bâtiment différent, équipé d'une petite baie informatique, de quoi y installer un peu de matériel réseau ! La baie du bâtiment R&D sera relié à la salle serveur avec une fibre optique.
...
Voici les sous-réseaux minimums à créer :
- **Paris :
...
Une zone démilitarisée (DMZ) est un sous-réseau un peu particulier : les machines dans ces sous-réseaux sont isolées du LAN par des règles de pare-feu spécifiques. Pas de panique, on ne va pas configurer de pare-feu dans cet atelier, on y reviendra plus tard dans la saison.
...
Pour le WiFi, on ne s’embête pas : on connectera directement les machines sur le switch 2960-24TT en filaire.
...
**Note
Internet, ça ne fonctionne pas comme ça ! Mais pour les besoins de cet atelier on va faire comme ça.
...
**Bonus**
Déjà fini ? Si vous avez encore du temps et de l'énergie, vous pouvez essayer de remplacer les switchs 2960 utilisés pour le WiFi par des point d'accès WiFi !
...
Devoir configurer DHCP sur chaque routeur est un peu galère, ce serait bien qu'on puisse tout configurer sur un serveur central (dans la DMZ, par exemple), avec plusieurs pools pour chaque sous-réseau ! Problème, les trames DHCP sont en broadcast, et ne traversent donc pas les routeurs...

Il faudrait trouver un moyen de _relayer_ les trames DHCP vers notre serveur 🤔

# Solution :
## Étape 1 – Plan d’adressage
Pour éviter des éventuels chevauchements avec les réseaux domestiques des télétravailleurs (192.168.x.x), je propose un plan d’adressage **10.10.0.0/16** (du RFC 1918), me donnant **65.534** adresses IP disponibles (**2^ [32 – 16] - 2**). Ci-dessous ma proposition de plan évolutif et résumable par site :
### Paris – bloc résumé : 10.10.0.0/16
- **LAN** (PC fixes + portables) : **10.10.0.0/20** (4.094 adresses)
  avec **10.10.0.1** comme **passerelle prévue**
- **DMZ** (serveurs exposés) : **10.10.16.0/24** (254 adresses)
  avec **10.10.16.1** comme **passerelle prévue**
- **WiFi public** (visiteurs uniquement) : **10.10.32.0/23** (510 adresses)
  avec **10.10.32.1** comme **passerelle prévue**

Cela me laisse une réserve pour croissance du genre :
- **10.10.128.0/18** /128.0-191.255) pour VLAN
- **10.10.17.0/24** à **31.0/24** (hors 16.0/24) pour DMZ
- **10.10.34.0/23** pour le WiFi
### Lille – bloc résumé : 10.20.0.0/16
- **LAN** (PC fixes + portables) : **10.20.0.0/20** (4.094 adresses)
  avec **10.20.0.1** comme **passerelle prévue**
- **WiFi** **public** (visiteurs uniquement) : **10.20.32.0/23** (510 adresses)
  avec **10.20.32.1** comme **passerelle prévue**
- **VPN** (postes collaborateurs distants) : **10.20.64.0/23** (510 adresses)
  avec **10.20.64.1** comme **passerelle prévue**
Le **LAN** entre les **trois routeurs** peut avoir l’ip 172.16.0.0/24
## Étape 2 – Câblage
Je relie les sites
![[Pasted image 20251107161030.png]]
