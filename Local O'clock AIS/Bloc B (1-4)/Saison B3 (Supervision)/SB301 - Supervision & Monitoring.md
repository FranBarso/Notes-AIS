Cours du 19/01/2026, avec Patrice Maldi
...
## Pourquoi superviser un système informatique ?
...
Les **accords de niveau de service** (**SLA**s) définissent les seuils de performance acceptables.
...
Le **monitoring centralisé** regroupe toutes les données dans un seul outil de supervision, tandis que le **monitoring décentralisé** distribue l'analyse entre plusieurs systèmes pour réduire la charge et améliorer la résilience.
...
## Supervision en environnement cloud et hybride
...
Les bases de données spécialisées, comme les **Time Séries Databases** (**TSDB**s), permettent de stocker et analyser efficacement les métriques de supervision.

Voir sur **Timestamp** (l’horodatage) : [https://en.wikipedia.org/wiki/Timestamp](https://en.wikipedia.org/wiki/Timestamp)

Et ses convertisseurs : [https://www.timestamp.fr/](https://www.timestamp.fr/)
...
"Le langage universel de la supervision réseau"
...
**Exemples** : Zabbix (communautaire + open source), PRTG, Centreon (open source) et Nagios (gratuit).                                                                      

[https://www.centreon.com/fr/centreon-infra-monitoring/](https://www.centreon.com/fr/centreon-infra-monitoring/)
...
Exemple d'OlD
![[Pasted image 20260119135413.png]]

Les OID suivent une structure arborescente standardisée ISO
[https://www.it-connect.fr/snmpv2c-vs-snmpv3-comparaison-de-la-securite-avec-wireshark/](https://www.it-connect.fr/snmpv2c-vs-snmpv3-comparaison-de-la-securite-avec-wireshark/)
## Les opérations SNMP
...
![[Pasted image 20260119140421.png]]
Communication synchronisée

### TRAP Notification

L'agent envoie spontanément une alerte :
![[Pasted image 20260119140450.png]]
Communication asynchrone : pas d'attente de réponse

### GET vs TRAP : quelle différence ?
![[Pasted image 20260119140521.png]]
Une bonne supervision combine les deux approches !
...
SNMP utilise **UDP** (non fiable) plutôt que TCP pour des raisons de performance.
...
Obsolète aujourd'hui mais encore présent sur du matériel ancien.
...
Version la plus utilisée actuellement par simplicité.

## Comparaison des versions

![[Pasted image 20260119151035.png]]
## Sécurité
...
**Problème** : transmis en **clair** sur le réseau !
...
Un attaquant peut **espionner** et **modifier** les équipements !
...
Voir la doc de Zabbix : [https://www.zabbix.com/documentation/8.0/en/manual/appliance](https://www.zabbix.com/documentation/8.0/en/manual/appliance)
...
- **Standardise** : fonctionne sur tous les équipements réseau
- **Leger** : peu de charge sur les équipements
...
- X **Sécurité** : v1/v2c non sécurisés
- X **UDP** : perte de paquets possible
- X **Complexité** : MIB et OID difficiles à maîtriser
- X **Pas de garantie de livraison** : TRAPs peuvent être perdus
- X **Limité au réseau** : peu adapté aux applications métier

SNMP est excellent pour le **réseau**, mais insuffisant seul pour une supervision complète.
...
Une stratégie de supervision efficace **combine plusieurs approches**.
