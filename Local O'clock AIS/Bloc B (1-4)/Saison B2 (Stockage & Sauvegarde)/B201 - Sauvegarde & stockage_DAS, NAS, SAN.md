Cours du 12-01-2026
Primordial en sécurité info : il faut faire des sauvegardes

Et il faut bien les stocker

## Introduction

Primordial en sécurité ?

La **sécurité absolue (parfaite, inviolable) n'existe pas**, aucun système n'est sécurisé à 100%.

Il faut **se préparer à l'échec des mesures de sécurité** que nous avons mis en place.

Prenons un cas de figure très courant :

**une attaque par ransomware** (rançongiciel FR) !

Le serveur de fichiers de l'entreprise est touché Toutes les données des utilisateurs sont stockées dessus, les **informations vitales à l'activité de l'entreprise** ne sont plus accessibles ... quelles solutions s'offrent à nous ?

Voir exemple de Blackshades : [https://en.wikipedia.org/wiki/Blackshades](https://en.wikipedia.org/wiki/Blackshades)
![[Pasted image 20260112110210.png]]
Il y a un site français qui liste les domaines malicieux : [https://red.flag.domains/](https://red.flag.domains/)
...
[https://messervices.cyber.gouv.fr/guides/fondamentaux-sauvegarde-systemes-dinformation](https://messervices.cyber.gouv.fr/guides/fondamentaux-sauvegarde-systemes-dinformation)

et aussi pour les authentifications : [https://messervices.cyber.gouv.fr/guides/recommandations-relatives-lauthentification-multifacteur-et-aux-mots-de-passe](https://messervices.cyber.gouv.fr/guides/recommandations-relatives-lauthentification-multifacteur-et-aux-mots-de-passe)
...
PRA pas pareil que PRI (P Reprise Informatique, pour juste le système et non pas l’activité)

PCA pas pareil que PCI.
...
Facile la sauvegarde, un copier/coller sur une clé USB et hop terminé. Eh bien, non !

La création d'un cluster et la gestion de la haute disponibilité fait partie d'un plan de continuité (PCI, sur ESXi ou Proxmox). Video sur la mise en pratique : [https://www.youtube.com/watch?v=NI353Ev9TNc](https://www.youtube.com/watch?v=NI353Ev9TNc)
...
![[Pasted image 20260112110254.png]]
...
![[Pasted image 20260112110325.png]]
**Exemple** : _une sauvegarde en interne sur un serveur différent, et une sauvegarde sur bande à conserver hors-site._
![[Pasted image 20260112110352.png]]
...
La durée de rétention, c'est le nombre de jours (ou de mois/années) pendant lequel on va devoir **conserver une sauvegarde**. (_et après, on pourra supprimer la sauvegarde pour libérer de l'espace_). Cela sera toujours lié à l’activité (hôtellerie, pharmacie, etc.).
...
![[Pasted image 20260112110712.png]]
### Sauvegarde différentielle
...
## Médias de stockage
Voir : [https://www.hpe.com/fr/fr/what-is/san-storage.html](https://www.hpe.com/fr/fr/what-is/san-storage.html)
...
Chaque media a des avantages et des inconvénients, il n'y a pas un média parfait.
...
### DAS
Le Directly Attached Storage (DAS) est le type de stockage que nous utilisons le plus souvent : le média (disque dur, clé USB, etc.) est **directement connecté/attaché à une machine**.

Très rapide, il a l'inconvénient de ne **pas** être **accessible aux autres** **machines** sur le réseau, comme c’est local.
![[Pasted image 20260112120622.png]]
### NAS
Un serveur NAS (Network Attached Storage) est un serveur spécifiquement conçu pour le stockage de données.
...
![[Pasted image 20260112120609.png]]

### SAN
Un SAN (Storage Area Network) est un **réseau de stockage**. Plus exactement, ce sont des **baies de stockage** directement accessibles en mode bloc (comme en DAS) par le système de fichiers des serveurs.

C'est comme si on avait un disque dur en DAS, mais physiquement connecté à plusieurs serveurs. C’est de la fibre.
![[Pasted image 20260112120549.png]]
## RAID

Le RAID (Redundant Array of Independent Disks) est un ensemble de techniques de virtualisation du stockage permettant de **repartir des données sur plusieurs disques durs** afin d'améliorer les **performances**, la **sécurité** ou la **tolérance aux pannes**.

[https://www.inmac-wstore.com/broadcom-megaraid-mr408i-o-controleur-de-stockage-sata-6gb-s-sas-12gb-s-pcie-4-0-nvme-pcie-4-0-x8/p7343722.htm](https://www.inmac-wstore.com/broadcom-megaraid-mr408i-o-controleur-de-stockage-sata-6gb-s-sas-12gb-s-pcie-4-0-nvme-pcie-4-0-x8/p7343722.htm)
...
![[Pasted image 20260112134146.png]]
...
Sur le marché, pour le stockage & la sauvegarde !
### Solutions stockage
...
- IBM
---------------------------------------------
Culture sur les écoutes : [https://www.youtube.com/watch?v=0lz2KRRGQZI](https://www.youtube.com/watch?v=0lz2KRRGQZI)

[https://www.youtube.com/watch?v=CT72czY9MBE](https://www.youtube.com/watch?v=CT72czY9MBE)

Winnie l’Ourson : [https://www.marianne.net/monde/winnie-l-ourson-nouveau-censure-en-chine-pour-sa-ressemblance-avec-xi-jinping](https://www.marianne.net/monde/winnie-l-ourson-nouveau-censure-en-chine-pour-sa-ressemblance-avec-xi-jinping)
